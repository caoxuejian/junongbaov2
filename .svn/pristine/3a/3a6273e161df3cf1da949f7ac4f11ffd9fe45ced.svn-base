...
public class DragImageView extends ImageView {
	...
	void onTouchMove(MotionEvent event) {
		...
		if (mode == MODE.DRAG) {
			...
			com.nxt.nxtapp.common.LogUtil.syso(" current_x:" + current_x + " start_x:"
					+ start_x);
			/** 水平进行判断 **/
			if (isControl_H) {
				if (left >= 0) {
			...
			com.nxt.nxtapp.common.LogUtil.syso("tuodaong left:" + left + " right:" + right);
			/** 垂直判断 **/
			if (isControl_V) {
				if (top >= 0) {
					top = 0;
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			...
			if (current_Top <= 0 && current_Bottom >= screen_H) {
				// com.nxt.nxtapp.common.LogUtil.LogE("jj", "屏幕高度=" + this.getHeight());
			...
			if (current_Left <= 0 && current_Right >= screen_W) {
				...
				com.nxt.nxtapp.common.LogUtil.syso("fangda current_Left:" + current_Left
		...
		// 缩小
			...
			current_Right = this.getRight() - disX;
			current_Bottom = this.getBottom() - disY;
			com.nxt.nxtapp.common.LogUtil.syso("suoxiao current_Left:" + current_Left
					+ " current_Right:" + current_Right);
			/***
			 * 在这里要进行缩放处理
			 */
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			while (current_Width <= screen_W) {
				...
				com.nxt.nxtapp.common.LogUtil.syso("jj : top=" + top + ",bottom=" + bottom
